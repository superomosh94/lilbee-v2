    ...
    const renderAdmin = async () => {
        ...
        if (tableBody) {
            ...
            users.forEach(user => {
                ...
                row.innerHTML = `
                    <td style="padding: 16px; border-bottom: 1px solid rgba(255,255,255,0.05); font-size: 14px;">${user.email}</td>
                    <td style="padding: 16px; border-bottom: 1px solid rgba(255,255,255,0.05);"><span class="badge" style="background:rgba(255,184,107,0.1); color:var(--accent);">${user.role || 'user'}</span></td>
                    <td style="padding: 16px; border-bottom: 1px solid rgba(255,255,255,0.05);"><span class="status-badge ${user.isBanned ? 'status-completed' : 'status-active'}" style="background:${user.isBanned ? 'rgba(255,68,68,0.1)' : 'rgba(68,255,68,0.1)'}; color:${user.isBanned ? '#ff4444' : '#44ff44'}">${user.isBanned ? 'Banned' : 'Active'}</span></td>
                    <td style="padding: 16px; border-bottom: 1px solid rgba(255,255,255,0.05); text-align: right;">
                        <button class="small-btn" onclick="window.updateUserRole('${user.uid}', 'admin')">Admin</button>
                        <button class="small-btn" onclick="window.updateUserRole('${user.uid}', 'user')">User</button>
                        <button class="small-btn" style="color:var(--accent);" onclick="window.initiateChat('${user.uid}', '${user.email}')">Msg</button>
                        <button class="small-btn" style="color:${user.isBanned ? '#4caf50' : '#e74c3c'};" onclick="window.toggleUserBan('${user.uid}')">
